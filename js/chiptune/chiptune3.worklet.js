	...
	handleMessage_(msg) {
		...
		switch (msg.data.cmd) {
			...
			case 'setStereoSeparation':
				...
				break
			case 'setInterpolationFilter':
				this.config.interpolationFilter = v
				if (!this.modulePtr) return
				libopenmpt._openmpt_module_set_render_param(this.modulePtr, OPENMPT_MODULE_RENDER_INTERPOLATIONFILTER_LENGTH, v)
				break
			case 'setPitch':
