...
// Child window tracking (monitoring, parameters, etc.)
// Delegated to WindowManager - this reference allows access if needed
...
async function createEngineWindow(options = {}) {
    ...
    try {
        engineWindow = await helper.tools.browserWindow('default', {
            ...
            focusable: false,      // Never take focus - prevents stealing focus from player
            skipTaskbar: true,     // Hide from taskbar - it's a background service window
            width: 400,
            height: 300,
            resizable: false,
            maximizable: false,
        ...
        // DEBUG: Verify focusable setting and log initial state
        console.log('[MAIN] Engine window created - focusable: false');
        console.log('[MAIN] Engine window has focus:', engineWindow.isFocused());
        ...
        // DEBUG: Track if engine ever gets focus (should never happen)
        engineWindow.on('focus', () => {
            logger.warn('main', 'ENGINE WINDOW FOCUSED in main process!');
            console.error('[MAIN] ENGINE FOCUS EVENT - Forcing focus back to player');
...
function setupAudioIPC() {
    // Safety: Engine window should never receive focus. If it does, redirect to player.
    ipcMain.on('engine:focus-steal', () => {
        logger.warn('main', 'Engine focus steal detected - redirecting to player');
        if (wins.main && !wins.main.isDestroyed()) {
            wins.main.focus();
        }
    });
    ...
    ipcMain.on('window-created', (e, data) => {
        ...
        if (data && data.type) {
            ...
            logger.info('window', 'window-created', { type, windowId });

            // Track in main state for engine restoration
            if (childWindows[type]) {
                childWindows[type].open = true;
                childWindows[type].windowId = windowId;
                childWindows[type].visible = true;  // Window is created and shown
            }
            sendToEngine('window-created', data);

            // Send correct stageId (engine window ID) to child window
            if (engineWindow && !engineWindow.isDestroyed() && windowId) {
                const childWin = BrowserWindow.fromId(windowId);
                if (childWin && !childWin.isDestroyed()) {
                    childWin.webContents.send('update-stage-id', { stageId: engineWindow.id });

                    // ATTACH NATIVE EVENT LISTENERS
                    // This is the most reliable way to handle focus restoration

                    // Remove existing listeners to avoid duplicates if window is recreated with same ID (unlikely but safe)
                    childWin.removeAllListeners('hide');
                    // childWin.removeAllListeners('closed'); // 'closed' is terminal anyway

                    childWin.on('hide', () => {
                        logger.info('window', 'Native window hide event', { type, windowId });
                        // Update state
                        if (childWindows[type]) childWindows[type].visible = false;
                        ...
                        if (wins.main && !wins.main.isDestroyed()) {
                            ...
                            logger.debug('window', 'Restoring focus (native hide handler)', { isMinimized, isVisible });

                            // Small timeout to allow the hide animation/logic to settle
                            setTimeout(() => {
                                ...
                                if (wins.main.isMinimized()) {
                                    wins.main.restore();
                                }

                                // 3. Force Focus (Windows hack often needed)
                                if (process.platform === 'win32') {
                                    wins.main.setAlwaysOnTop(true);
                                    wins.main.show(); // 'show' also focuses
                                    wins.main.focus();
                                    wins.main.setAlwaysOnTop(false);
                    ...
                    childWin.on('close', () => {
                        logger.info('window', 'Native window close event', { type, windowId });
                        // State update handled by window-closed IPC usually, but good to have here too
                    });
                }
            }
    ...
    ipcMain.on('window-hidden', (e, data) => {
        ...
        logger.info('window', 'window-hidden received', { type, windowId });
    ...
    ipcMain.on('window-closed', (e, data) => {
        ...
        logger.info('window', 'window-closed received', { type, windowId });

        // window-closed - window is destroyed (only Mixer does this, others hide)
        if (data && data.type) {
    ...
    ipcMain.on('window:toggle', (e, data) => {
        ...
        if (!winState.windowId) {
            ...
            logger.debug('window', 'Window does not exist, requesting creation', { type });
            ...
            try {
                ...
                if (win && !win.isDestroyed()) {
                    logger.debug('window', 'Hiding window and focusing player', { type });
                    // Explicitly blur first to help OS focus switch
                    win.blur();
                    win.hide();
                    ...
                    if (wins.main && !wins.main.isDestroyed()) {
                        setTimeout(() => {
                            try {
                                ...
                                logger.error('window', 'Error focusing player', err);
                            }
                        }, 50);
                    }
                }
            } catch (err) {
                logger.error('window', 'Error toggling window (hide)', err);
            }
        } else {
            ...
            try {
                const win = BrowserWindow.fromId(winState.windowId);
                if (win && !win.isDestroyed()) {
                    logger.debug('window', 'Showing window', { type });
                    win.show();
                    win.focus();
                ...
                logger.error('window', 'Error toggling window (show)', err);
            }
        }
    });
