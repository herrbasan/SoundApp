...
class OfflinePitchShiftProcessor extends AudioWorkletProcessor {
  ...
  constructor() {
    super()
    this.port.onmessage = ({ data }: MessageEvent<any>) => {
      const { event } = data
      switch (event) {
        case 'buffer': {
    ...
    createModule()
      .then((module) => {
        const rbModule = module as unknown as RubberBandModule
        const init = () => {
          const moduleAny: any = rbModule as any
          if (!(moduleAny && typeof moduleAny._malloc === 'function' && moduleAny.HEAPF32)) {
            // Module isn't fully initialized yet; try again next render quantum.
            return
          }
      ...
      .catch((err) => {
        console.error('OfflinePitchShiftProcessor: createModule failed', err)
      })
  }
